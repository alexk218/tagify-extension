(function tagMasterMenu() {
    // Wait for all required Spicetify components to be available
    if (!(Spicetify.CosmosAsync && Spicetify.Platform && Spicetify.ContextMenu)) {
        setTimeout(tagMasterMenu, 300);
        return;
    }

    console.log("TagMaster Menu Extension loading...");
    ...
    function handleTagMenuClick(uris) {
        console.log("TagMaster menu clicked with URIs:", uris);
        ...
        if (uris.length > 0) {
            ...
            // Use a different approach to navigate with state
    ...
    console.log("TagMaster menu registered successfully");
})();